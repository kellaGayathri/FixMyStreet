    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 text-slate-900 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Right Action: Student Project Guide, Auth Status & Mobile Hamburger */}
          <div className="flex items-center gap-2">
            <button
              id="about-docs-btn"
              type="button"
            ...
            >
              ...
              <span className="hidden lg:inline">Project Docs</span>
            </button>
            ...
            {currentUser ? (
              <div className="hidden sm:flex items-center gap-2 pl-2 border-l border-slate-200">
                <div
                  onClick={() => handleNavClick(currentUser.role === "admin" ? "admin-dashboard" : "citizen-dashboard")}
                  className="flex items-center gap-2 cursor-pointer group"
                  title="Go to your dashboard"
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition group-hover:scale-105 ${
          ...
          {currentUser ? (
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-slate-500">
                    {currentUser.role === "admin" ? "Municipal Official" : "Citizen"} • {currentUser.email}
                  </div>
                </div>
              </div>
              <button
                ...
                className="px-2.5 py-1 text-xs text-red-600 hover:bg-red-50 rounded-md font-semibold"
              >
                Sign Out
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pb-2 mb-2 border-b border-slate-100">
              <button
                type="button"
                onClick={() => handleNavClick("login")}
                className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-800"
              >
              ...
              <button
                type="button"
                onClick={() => handleNavClick("register")}
                className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white"
              >
                <UserPlus className="w-3.5 h-3.5" />
                Register
              </button>
